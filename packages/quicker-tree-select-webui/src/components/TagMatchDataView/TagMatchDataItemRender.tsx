...
import type { ArrayDataSourceType, DataSourceType, ObjectDataSourceType } from 'quicker-tree-select-core/types'
...
export const TagMatchDataItemRenderWrapper: React.FC<{
  ...
  return (
    <Card variant="default" className="flex flex-col w-full min-w-[180px] max-w-full">
...
export const TagMatchDataItemRender: React.FC<TagMatchDataItemRenderProps> = ({ value }) => {
  /** 点击后触发事件 */
    ...
    return (
      <ListBox>
        {itemAssert.map((item, index) => (
          <ListBox.Item key={`${item}_${index}`} id={`${item}_${index}`} textValue={`${item}_${index}`}>
      ...
      <ListBox>
        {itemEntries.map(([key, value]) => (
          <ListBox.Item key={key} id={key} textValue={value}>
            <div className="flex flex-col" onClick={() => triggerClickFn([key, value])}>
              <Label>{key}</Label>
