 ...
 */
export function initDataWithState(_dataSource: DataSourceType[], dataWithState?: DataWithState | null) {
  const dataSource = dataWithState?.dataSource ? dataWithState.dataSource : _dataSource
  ...
  const updateDataResult = (): DataWithState => {
    ...
    result.selectedTagsMatchedData = dataSource.filter(item => state.selectedTags.every(tag => item.tags.includes(tag)))
    // 剩余可选tag
    result.restRelatedTags = [
      ...new Set(
        dataSource
          .filter(item => state.selectedTags.every(tag => item.tags.includes(tag)))
          .map(item => item.tags)
          .flat()
  ...
  const removeTag = (tags: string[]) => {
    stateData.state.selectedTags = stateData.state.selectedTags.filter(item => !tags.includes(item))
    return updateDataResult()
  }
